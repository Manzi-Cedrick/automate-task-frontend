...
const CreateRole = ({ showModal, onClose }: { showModal: Boolean, onClose: any }) => {
    ...
    const handleOnSave = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        try {
            let res = await service.createRole(formData);
            if (res) {
            ...
            <div className='bg-white text-black py-2 px-10 w-[36vw] rounded-sm'>
                ...
                <div className='modal-body text-slate-500'>
                    <div>
                        ...
                        <input value={formData.value} onChange={(e) => { setFormData({ ...formData, value: e.target.value }) }} className="shadow appearance-none bg-inputG border rounded w-full py-3 text-[10px] px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="username" type="text" placeholder="Enter role name" />
                    </div>
                    <div>
                        <label className="block text-gray-700 text-[12px] py-2">
                            Fee
                        </label>
                        <input value={formData.fee} onChange={(e) => { setFormData({ ...formData, fee: e.target.valueAsNumber }) }} className="shadow appearance-none bg-inputG border rounded w-full py-3 text-[10px] px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="username" type="number" placeholder="Enter fee" />
                    </div>
                    <div>
